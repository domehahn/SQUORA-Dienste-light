// Erzeugt einen Login-Nutzer für die Dienste-App und gibt das fertige
...
//   wrangler d1 execute dienste --local --command "INSERT INTO users ..."
// oder --remote für die Produktions-Datenbank.
...
console.log(`wrangler d1 execute dienste --local --command "${sql.replace(/"/g, '\\"')}"`);
console.log(`wrangler d1 execute dienste --remote --command "${sql.replace(/"/g, '\\"')}"`);
