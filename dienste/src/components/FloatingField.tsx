...
export function FloatingInput({ label, id, ...props }: FloatingInputProps) {
  ...
  return (
    <div>
      ...
      <input
        id={inputId}
        className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm text-slate-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:[color-scheme:dark]"
        {...props}
      />
    </div>
  );
...
export function FloatingSelect({ label, id, children, ...props }: FloatingSelectProps) {
  ...
  return (
    <div>
      ...
      <select
        id={selectId}
        className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100"
        {...props}
      >
        {children}
      </select>
