...
// Gruppiert wie im Schwesterprojekt SQUORA-Turnen-light: feste linke
// Seitenspalte statt einer mit vielen Punkten überladenen horizontalen Zeile.
const NAV_GROUPS: { label: string; items: { to: string; label: string; adminOnly?: boolean }[] }[] = [
  {
    label: "Turniere",
    items: [
      { to: "/admin/turniere", label: "Turniere" },
      { to: "/admin/kasse", label: "Kasse" },
      { to: "/admin/kassenbuch", label: "Kassenbuch" },
    ],
  },
  {
    label: "Stammdaten",
        ...
        return (
          <div key={group.label} className="mb-5">
            ...
            <div className="space-y-0.5">
              {items.map((item) => (
                <NavLink key={item.to} to={item.to} className={navLinkClass} onClick={() => setNavOpen(false)}>
                  {item.label}
                </NavLink>
              ))}
            </div>
  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-3">
          <div className="flex items-center gap-2">
            <button
              ...
              className="rounded-md border border-slate-300 p-2 text-slate-600 hover:bg-slate-100 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800 print:hidden sm:hidden"
            >
      ...
      <div className="mx-auto flex max-w-6xl gap-6 px-4 py-6">
        {/* Mobil: ausklappbare Overlay-Navigation samt Backdrop; ab sm: feste
            Seitenspalte links. */}
        {navOpen && (
          <div className="fixed inset-0 z-30 bg-black/30 sm:hidden" onClick={() => setNavOpen(false)} aria-hidden="true" />
        )}
        <nav
          className={`fixed inset-y-0 left-0 z-40 w-64 overflow-y-auto border-r border-slate-200 bg-white p-4 transition-transform dark:border-slate-800 dark:bg-slate-900 print:hidden sm:static sm:z-auto sm:w-48 sm:shrink-0 sm:translate-x-0 sm:border-r-0 sm:bg-transparent sm:p-0 sm:dark:bg-transparent ${
