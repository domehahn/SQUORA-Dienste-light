...
// SQUORA-Turnen-light/turnen/src/components/SquoraBrand.tsx) - Icon +
// Wortmarke mit Farbverlauf, "Dienste" als App-Namenszusatz darunter.
export default function SquoraBrand({ size = "xs", layout = "row", className = "" }: SquoraBrandProps) {
  ...
  const textBlock = (
    <div className={layout === "stack" ? "text-center leading-tight" : "leading-tight"}>
      <span
        className={`${wordmarkSize} font-extrabold tracking-tight bg-gradient-to-r from-blue-800 via-blue-600 to-teal-500 bg-clip-text text-transparent print:bg-none print:text-blue-800`}
      >
        SQUORA
      </span>
      <p className="text-xs font-medium text-slate-500 dark:text-slate-400">Dienste</p>
    </div>
  );
